# awesome-safety-tools landscape analysis — vs the fight-csam / FightCSAM portfolio
...
**Subject:** our 11 OSS tools (hashkit, hashkit-match, detectkit-test, csam-shield, promptshield, hashstream, trainguard, cybertip-cli, evidencevault, c2pa-lite, safemod) — packages keep the digitalharm naming; FightCSAM is the developer-site brand.
...
- **Cross-cutting (Osprey/Coop)** — Ship a **first-class ROOST Osprey adapter + Coop signal schema** so every detector verdict (csam-shield, hashkit-match, hashstream, promptshield) becomes an Osprey signal/UDF. Position FightCSAM as "the CSAM engines that feed your Osprey," not as a rules engine.
...
2. **A declarative actioner/rules integration** — not a rules engine of our own (Osprey owns that), but the **Osprey/Coop adapter** above, packaged as a shippable connector so a FightCSAM platform has something wiring verdicts→actions on day one. Today we ship detection with no path to action.
