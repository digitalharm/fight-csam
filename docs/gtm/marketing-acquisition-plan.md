...
**HARD LIMIT:** no mass/automated GitHub issue-or-PR spam across the 113. Do-not-contact list = all 13 out-of-scope + all 39 reference-only (many anonymous/low-provenance dataset dumps) + Big-Tech in-house T&S. Outreach restricted to the 25 "use" + a curated "learn-from" subset, only where there is genuine value to send. (See Guardrails.)
...
- **Outreach discipline (hard caps):** **≤5 hand-written messages/week**, capped to the **25 "use"-verdict** allies + a curated learn-from subset; **gitignored outreach ledger** (who/when/response/opt-out); **0** automated/bulk issues across the 113; **0** contacts to the do-not-contact list (13 out-of-scope + 39 reference-only + Big-Tech in-house T&S).
...
- **NO mass/automated GitHub issues or PRs across the 113 profiled projects** (or Meta/ROOST repos). Ever. One scripted blast is reputationally fatal and could get the awesome-safety-tools PR rejected.
...
- Ship the **agent-native baseline** (`/llms.txt`, `/llms-full.txt`, raw-md, `/.well-known/fightcsam.json`, `/agents` page stating "no hash list / never compliant"). **Rewrite all 11 READMEs** to lead with the verified copy-paste install + a 5-line first-value snippet. Set GitHub topics.
