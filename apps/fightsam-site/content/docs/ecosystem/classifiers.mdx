...
description: "ML classifiers and LLM guardrails. FightCSAM ships no general model — csam-shield is built to wrap the best of these as swappable detector backends, and…"
---
...
**23 projects** — 10 use · 10 learn from · 2 reference · 1 out of scope.
...
### [Realm OmniGuard](https://www.realmlabs.ai/)

**Use** · by Realm Labs · pairs with [promptshield](/docs/promptshield)

The same choke-point role as Guardrails AI or OpenGuardrails, but multimodal and vendor-supported: a team already running OmniGuard in front of its models can add promptshield's narrow CSAM-intent gate as one policy inside that firewall, and csam-shield can wrap OmniGuard's image/video verdicts as a custom detector backend beside hash matching. Closed and commercial, with no public SDK we could verify and no CSAM-specific claim in its published material, so it never replaces dedicated CSAM signals or hash matching; it is the enforcement surface those signals plug into.

> A commercial AI firewall from Realm Labs that screens inputs and outputs for harmful content, jailbreaks, and prompt injection across text, image, audio, and video in 50+ languages, deployable inside your own environment (Docker / Triton) or in front of an AI gateway.
