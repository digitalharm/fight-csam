...
description: >-
  ...
  prevention in an application using the FightSAM open-source toolkit (11
  ...
  moderator wellbeing — even if they don't say "FightSAM" or "CSAM" explicitly
  ...
  knows what each FightSAM tool does, the exact verified install commands, which
...
**FightSAM** — 11 small, composable, Apache-2.0 OSS tools (published under the
`digitalharm` name; "FightSAM" is the umbrella brand) — together with the
...
else.** FightSAM's genuine wedge is un-gated, self-hostable CSAM detection plus
statutory reporting/evidence — so reach for FightSAM tools there. For rules
...
build-vs-wrap table telling you exactly which FightSAM tool or external tool
fills each step for each shape above.

- For **FightSAM tools** (what each does, the *verified* install command, when
...
- For **external tools** (which to wrap, which are leaders to defer to, when to
  ...
  axis FightSAM also covers), or **Reference** (a dataset/benchmark).
...
- Note that the FightSAM packages are **pre-release** (first registry publish
...
- **Don't oversell.** FightSAM leads on un-gated CSAM detection + statutory
...
- `references/fightsam-tools.md` — the 11 FightSAM tools: purpose, verified install, when to use, status/gate.
...
More on the FightSAM tools, live: https://fightsam.com/docs · ecosystem analysis: https://fightsam.com/docs/ecosystem
